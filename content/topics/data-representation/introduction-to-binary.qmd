...
The steps to convert a binary number to decimal is straightforward 📝: 

1. 4️⃣ Find the power of 2 value for each bit of the binary number, starting from 0.(e.g., the power of 2 value for the 5th bit of a binary numberis: 2^5^=32, etc.)

2. 1️⃣ Add all the powers of two **wherever there is a 1** in the bit of the binary number

3. ➕ Add those powers of two → this is the decimal!
...
2. ✍️ Write down the **quotient** and **remainder** of the division on the side.

3. 🔁 Repeat steps 1 and 2 on the outcome.

4. ⛔ Stop when the quotient becomes 0.

4. 📥 Read the remainders from bottom to top → this is the binary number.
...
---
# Range of numbers based on number of bits ↔️

When using a fixed number of bits, the total number of unique values we can represent is always limited by 2^𝑛^, where 𝑛 is the number of bits. This means that no matter how we arrange the bits, once all possible combinations are used, we cannot create any new unique numbers without adding more bits. For example, with 3 bits we can only represent 8 values (0–7 in decimal), and with 8 bits (1 byte) we are restricted to 256 values (0–255). This limitation is why larger data types like 2 bytes (16 bits) or 4 bytes (32 bits) are needed when working with bigger numbers.
...
-   **n bits ** → 4
-   **Can represent 2^n^*** → 16 
-   **Values: from 0 to (2^n^ -1)** → 0 to 15
-   **First Number** 0000 = (0)~10~
-   **Last Number** 1111 = 16-1 = 15
